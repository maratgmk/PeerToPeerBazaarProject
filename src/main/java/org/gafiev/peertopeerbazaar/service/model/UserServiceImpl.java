...
public class UserServiceImpl implements UserService {
    ...
    /**
     * получение пользователя из БД вместе с его множеством платёжных аккаунтов
     * @param id идентификатор пользователя
     * @return DTO пользователя с подтягиванием ленивых частей
     */
    @Override
    public UserResponse getUserByIdWithPaymentAccounts(Long id) {
        User user =  userRepository.findByIdWithPaymentAccounts(id)
                .orElseThrow(() -> new EntityNotFoundException(User.class,Map.of("id", String.valueOf(id))));
        return userMapper.toUserResponse(user);
    }
